...
class TestTransformPiecewiseModelToAccidentalReducedInnerRepnGDP(unittest.TestCase):
    def check_disjunct(self, d, pts):
    ...
    def check_pw_log(self, m):
        ...
        disjuncts_dict = {
            # disjunct : [extreme points in corresponding x domain]
            log_block.disjuncts[0]: (0, 1),
            log_block.disjuncts[1]: (1, 2),
            log_block.disjuncts[2]: (2, 3),
        }
        for d, pts in disjuncts_dict.items():
            self.check_disjunct(d, pts)
    ...
    def check_pw_paraboloid(self, m):
        ...
        disjuncts_dict = {
            ...
            paraboloid_block.disjuncts[0]: [0, 1, 2],
            paraboloid_block.disjuncts[1]: [0, 2, 3],
            paraboloid_block.disjuncts[2]: [2, 4, 5],
            paraboloid_block.disjuncts[3]: [1, 2, 5],
        }
        for d, pts in disjuncts_dict.items():
            self.check_disjunct(d, pts)
    ...
    def test_transformation_do_not_descend(self):
        ct.check_transformation_do_not_descend(
            self, 'contrib.piecewise.accidental_reduced_inner_repn_gdp'
        )
