...
from pyomo.common.config import ConfigDict, ConfigValue, PositiveInt, InEnum, In
...
class NonlinearToPWL(Transformation):
    ...
    CONFIG.declare(
        'triangulation',
        ConfigValue(
            default=Triangulation.Delaunay,
            domain=In(Triangulation),
            description="Triangulation to use for the domain of the piecewise-linear "
            "approximations",
            doc="""
            The argument for 'triangulation' that will be passed to the
            PiecewiseLinearFunction constructor when the approximations are created.
            """,
        ),
    )
